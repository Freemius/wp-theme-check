<?php
    ...
    class WordPress extends PrettyPrinterAbstract
    {
        // Special nodes
        ...
        protected function pAssociativeArray(array $nodes)
        {
            $key_max_len = 0;
            foreach ($nodes as $node)
            {
                if ( ! is_null($node->key))
                    $key_max_len = max($key_max_len, strlen($this->p($node->key)));
            }
            ...
            foreach ($nodes as $node)
            {
                ...
                $array_str .= (! is_null($node->key) ?
                        (str_pad($this->p($node->key), $key_max_len) . ' => ') :
                        '') .
                              ($node->byRef ? '&' : '') . $this->p($node->value) . ',';
            }

            return WordPress::INDENT . $array_str;
        }
        ...
        protected function pCommaSeparated(array $nodes)
        {
            ...
            else
            {
                ...
                {
                    // If more than 3 arguments, separate by end of line.
                    return "\n" . WordPress::INDENT . $this->pImplode($nodes, ",\n" . WordPress::INDENT) . "\n";
                }
            }
        }
        ...
        public function pExpr_Array(Expr\Array_ $node)
        {
            if (is_array($node->items) &&
                0 < count($node->items) &&
                ! is_null($node->items[0]->key)
            )
            {
                return "array(\n" . $this->pAssociativeArray($node->items) . "\n)";
            }
        ...
        {
            /**
             * Add line space if the if statement:
             *  1. Has more than one line in it.
             *  2. Or has else or elseif.
             *
             * @todo This method should be based on one node before and after, to prevent situations like:
             * function f(){
             *
             *       if (....){
             *       }
             ...
             * }
             */
            $line_space = (1 < (count($node->stmts) +
                                (0 < count($node->elseifs) ? 2 : 0) +
                                (! is_null($node->else) ? 2 : 0)
                ));
